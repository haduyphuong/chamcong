...
$nv = $conn->query("SELECT * FROM users");
...
if (isset($_SESSION['Ma_nv'])) {
    $manv = $_SESSION['Ma_nv'];
    $nv_ca = $conn->query("SELECT * FROM phan_ca_lam pcl JOIN ca_lam_viec clv ON pcl.id_calam = clv.ID WHERE pcl.Ma_nv = '$manv' AND pcl.ngay = CURDATE()");

    $nv_ca_bu = $conn->query("SELECT * FROM cham_bu  WHERE Ma_nv = '$manv' AND status = 1 AND Ngay_cham_bu = CURDATE()");

    $nv_ca_tang = $conn->query("SELECT * FROM tang_ca WHERE Ma_nv = '$manv' AND  status = 1 AND Ngay = CURDATE()");
...
<div class="content-wrapper">
    ...
    <?php if ($_SESSION['role'] == 'employee') { ?>
                ...
                <div class="row">
                    <div class="col-lg-6 col-6 mb-2">
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <?php  if($nv_ca->num_rows >0){while ($row = $nv_ca->fetch_assoc()) {
        ...
        $id_bophan_query = $conn->query("SELECT ID_bophan FROM users WHERE Ma_nv = '$manv'");
...
// WHERE pcl.Ma_nv = '$username' AND pcl.ngay = CURDATE()");
        ?>
            ...
            $maNV = $_SESSION['Ma_nv'];
            $sqlpbt = "SELECT ID FROM bo_phan WHERE email = '$email'";
