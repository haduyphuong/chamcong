...
$users = $conn->query("SELECT * FROM users WHERE ID_bophan = '$bo_phan_id'");



// Lấy thông tin tin nhắn giữa người dùng và người được chat từ bảng group_chat
$messages = $conn->query("SELECT gc.*, u.username AS sender_name FROM group_chat_nb gc JOIN users u ON gc.user_id = u.id JOIN bo_phan bp ON u.ID_bophan = bp.ID WHERE bp.ID = '$bo_phan_id'  ORDER BY gc.created_at ASC");
?>
                                ...
                                <?php if ($messages->num_rows > 0): ?>
                                    <?php while ($row = $messages->fetch_assoc()): ?>
                                        <li class="chat-message <?php echo ($row['user_id'] == $user_id) ? 'sent' : 'received'; ?>">
                                            <div class="media">
                                                <img src="/admin/Public/icon_chat.jpg" alt="<?php echo htmlspecialchars($row['sender_name']); ?>" class="avatar mr-3" />
...
<style>
    ...
    .chat-messages {
        ...
        padding: 0;
        max-height: 400px; /* Chiều cao cố định cho khung tin nhắn */
    ...
    .chat-message.sent {
        justify-content: flex-end; /* Tin nhắn gửi nằm bên phải */
    }

    .chat-message.received {
        justify-content: flex-start; /* Tin nhắn nhận nằm bên trái */
    }

    .media-body {
        max-width: 100%; /* Giới hạn chiều rộng hộp tin nhắn */
        padding: 10px;
        border-radius: 5px;
        position: relative;
        color: white; /* Màu chữ cho tin nhắn */
    }

    .chat-message.sent .media-body {
        background-color: #007bff; /* Màu nền cho tin nhắn gửi */
        margin-left: auto; /* Căn phải */
    }

    .chat-message.received .media-body {
        background-color: #f1f1f1; /* Màu nền cho tin nhắn nhận */
        color: black; /* Màu chữ cho tin nhắn nhận */
    ...
    .input-group {
        margin-top: auto; /* Đẩy phần input xuống dưới cùng */
    }
</style>
                ...
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    ...
                    if (response.success) {
                        ...
                        newMessage.innerHTML = `
                            <div class="media">
                                <img src="/admin/Public/icon_chat.jpg" alt="You" class="avatar mr-3" />
                                <div class="media-body">
                                    <p>${message}</p>
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    ...
                    response.forEach(function(row) {
                        ...
                        newMessage.innerHTML = `
                            <div class="media">
                                <img src="/admin/Public/icon_chat.jpg" alt="${row.sender_name}" class="avatar mr-3" />
                                <div class="media-body">
                                    <p>${row.message}</p>
