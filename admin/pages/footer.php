...
<?php
$messageSamples = isset($messageSamples) ? $messageSamples : [];
?>

const messageSamples = <?php echo json_encode($messageSamples); ?>;
...
document.getElementById('sendSelectedMessages').addEventListener('click', function() {
    ...
    fetch('save_message.php', {
        ...
        body: JSON.stringify({ messages: selectedMessages, userId: userId }),
    })
...
<style>
    ...
    .custom-card-header {
    border-bottom: 1px solid #ccc; /* Màu của đường viền */
    width: 99%; /* Chiều rộng của đường viền */
    margin: 0 auto; /* Căn giữa */
}
.card-body img {
    border: 2px solid #007bff; /* Đường viền cho hình ảnh */
}

.card-body p {
    margin: 0; /* Bỏ margin cho các thẻ p */
}
...
.bg-info {
    background-color: #17a2b8 !important; /* Màu nền cho tin nhắn admin */
}

.text-light {
    color: white !important; /* Màu chữ cho tin nhắn admin */
}

/* Tin nhắn của người gửi */
.user-message {
    background-color: #e9ecef !important; /* Màu nền cho tin nhắn của bạn */
    border-radius: 10px;
    margin-bottom: 10px;
    padding: 10px;
...
.admin-message {
    background-color: #17a2b8 !important; /* Màu nền cho tin nhắn admin */
    color: white; /* Màu chữ cho tin nhắn admin */
    border-radius: 10px;
    margin-bottom: 10px;
    padding: 10px;
    max-width: 70%; /* Giới hạn chiều rộng */
    align-self: flex-start; /* Đẩy sang bên trái */
}
...
#chatMessages {
    ...
    align-items: flex-start; /* Mặc định tin nhắn nằm bên trái */
}
