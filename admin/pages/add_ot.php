...
global $conn;
$ten_QL = $_SESSION['username'];
$sqlpb = "SELECT ID_bophan FROM bo_phan WHERE Tenql = '$ten_QL'";
$resultbp = $conn->query($sqlpb);
$row1 = $resultbp->fetch_assoc();
$Idbp = $row1['ID_bophan'];
$nhan_vien = $conn->query("SELECT * FROM users WHERE ID_bophan = '$Idbp'");
?>
                                    ...
                                    <select required class="form-control" name="employee" id="">
                                        ...
                                        if ($nhan_vien->num_rows > 0) {
                                            while ($row = $nhan_vien->fetch_assoc()) {
                                                echo '<option value="' . $row['Ma_nv'] . '">' . $row['username'] . '</option>';
                                            }
                                        }
                                        ?>
                                    </select>
