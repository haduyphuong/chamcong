...
global $conn;
$nhan_vien = $conn->query("SELECT * FROM users");
$bo_phan = $conn->query("SELECT * FROM bo_phan");
$ca_lam_viec = $conn->query("SELECT * FROM ca_lam_viec"); ?>
<?php
$manv = $_GET['Ma_nv'];
$sql = "SELECT u.*,b.Ten  FROM users u JOIN bo_phan b ON u.ID_bophan = b.ID  where u.Ma_nv ='$manv'";
                            ...
                            <form method="post">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="bophan" id="bophan" value="<?php echo $bophan ?>"
                                                ...
                                                <?php while ($row = $bo_phan->fetch_assoc()): ?>
                                                    <option value="<?php echo $row['ID_bophan'] ?>"><?php echo $row['ten_pb'] ?>
                                                    </option>
                                                <?php endwhile ?>
                                            </select>
                                        </div>
                                ...
                                <div class="row mb-2">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <label>Mã bộ</label>
                                            <input type="text" name="mabo" class="form-control" placeholder="Mã bộ"
                                                required value="<?php echo $mabo ?>">
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <label>Mã nhóm</label>
                                            <input type="text" name="manhom" class="form-control" placeholder="Mã nhóm"
                                                required value="<?php echo $manhom ?>">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-sm-6 text-right">
                                        <a onclick="return confirm('Bạn có chắc chắn muốn xóa nhân viên này? Mọi dữ liệu liên quan đến nhân viên sẽ bị xóa!');"
                                            href="xoa_nv.php?x=<?php echo $manv ?>" class="btn btn-danger">Xoá nhân
                                            viên</a>
                                    </div>
                                </div>
                                <?php
                                if (isset($_POST['btn'])) {
                                    ...
                                    $sql = "UPDATE users SET username='$ten', password='$password', email='$email', Gioitinh='$gt', ID_bophan='$bo_phan', chucdanh='$chucdanh', Ma_bo='$mabo', Ma_nhom='$manhom' WHERE Ma_nv= '$manv'";
                                    
                                    $result = mysqli_query($conn, $sql) or die("Có lỗi xảy ra: " . mysqli_error($conn));
                            
                                    echo '<script>
                                                window.location.href = "ql_nhan_vien.php";
                                                </script>';
                                }
                                ?>
                            </form>
