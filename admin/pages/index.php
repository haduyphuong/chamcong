...
<body>
    ...
    <form action="" method="post">
        ...
        <div class="inputGroup inputGroup1">
            <input type="text" id="email" class="email" name="email" maxlength="50" required placeholder="Email" />
            <span class="indicator"></span>
        </div>
        <div class="inputGroup inputGroup2">
            <input type="password" id="password" name="pass" class="password" maxlength="30" required placeholder="Password" autocomplete="on" />
        </div>
      
        <div class="inputGroup inputGroup3">
            <button type="submit" id="login" name="btn">Login</button>
        </div>
        ...
        if (isset($_POST['btn'])) {
            ...
            if ($num_rows != 0 ) {
                $row = mysqli_fetch_assoc($result);
                 ...
                 if ($row['role'] == 2 && $pass == $row['password'] && $row['status'] == 1) {
                    $otp = rand(100000, 999999);
                    $_SESSION['otp'] = $otp; // Lưu OTP vào session
                    $_SESSION['id'] = $row['id'];
                    ...
                    try {
                        ...
                        $mail->isSMTP();
                        $mail->Host = 'smtp.gmail.com'; // Địa chỉ máy chủ SMTP (ví dụ: smtp.gmail.com)
                        $mail->SMTPAuth = true;
                        $mail->Username = 'quantrichamcong@gmail.com'; // Địa chỉ email của bạn
                        $mail->Password = 'lhyndlshxpslppsb'; // Mật khẩu ứng dụng
                        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // Hoặc PHPMailer::ENCRYPTION_SMTPS
                        $mail->Port = 587; // Cổng SMTP
        
                        // Người gửi và người nhận
                        $mail->setFrom('quantrichamcong@gmail.com', 'Login OTP'); // Email và tên của bạn
                        $mail->addAddress($to); // Địa chỉ email của người nhận
                        $mail->addReplyTo('quantrichamcong@gmail.com', 'Login OTP'); // Địa chỉ trả lời
                ...
                }else if ($pass == $row['password']  && $row['status'] == 1) {
                    $_SESSION['id'] = $row['id'];
                    $_SESSION['username'] = $row['username'];
                ...
                else if($pass == $row['password'] && $row['status'] == 0){
