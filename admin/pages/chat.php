...
$messages = $conn->query("
    ...
    JOIN users u2 ON c.assign_to = u2.id
    WHERE c.id_group = $message_id AND c.status = 1
...
if ($message_id != null) {
    ...
    $stmt = $conn->prepare("SELECT assign_to FROM messages WHERE id = ?");
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            $assigned_to = $row['assign_to']; // Lấy giá trị assigned_to
        }
    } else {
                            ...
                            <div class="card-body card-body-chat">
                                ...
                                <ul class="chat-messages">
                                    <?php if ($messages_admin->num_rows > 0): ?>
                                        ...
                                        ?>
                                            <li class="chat-message <?php echo ($row['id_user'] == $user_id) ? 'sent' : 'received'; ?>">
                                                <div class="media">
                            ...
                            <div class="card-body card-body-chat">
                                ...
                                <ul class="chat-messages">
                                    <?php if ($messages_tp->num_rows > 0): ?>
                                        ...
                                        <?php while ($row = $messages_tp->fetch_assoc()): $row['assign_to'] ?>
                                            <li class="chat-message <?php echo ($row['id_user'] == $user_id) ? 'sent' : 'received'; ?>">
                                                <div class="media">
                                                    <img src="../Public/avatardefault.png" alt="<?php echo htmlspecialchars($row['sender_name']); ?>" class="avatar mr-3 icon_chat" />
                                                    <div class="media-body d-flex justify-content-between align-items-center">
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body card-body-chat">
                                ...
                                <?php
                                $id_m = $_GET['message'];
                                $sql_m = "SELECT status FROM messages WHERE id = $id_m";
                                $result_m = mysqli_query($conn, $sql_m);
                                $row_m = $result_m->fetch_assoc();
                                $isActiveM = $row_m['status'];
                                if ($isActiveM == 1) {
                                ?>
...
if (isset($_POST['btn'])) {
    ...
    $sql1 = "UPDATE messages SET status = 0 WHERE id = $id_message";
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('messageInput').addEventListener('keydown', function(event) {
            if (event.key === "Enter") {
                event.preventDefault(); // Ngăn chặn hành vi mặc định (ví dụ: xuống dòng)
                sendMessage();
            }
        });
