...
function search()
{
  ...
  if (isset($_GET['search']) && !empty($_GET['search'])) {
    $key = $_GET['search'];
    $sql = "SELECT * FROM users WHERE Ma_nv LIKE '%key%' OR Gioitinh  LIKE '%key%' OR Quequan  LIKE '%key%' OR ID_bophan  LIKE '%key%' OR ID_ca_lam  LIKE '%key%' OR He_so_luong  LIKE '%key%'";
  } else { {
